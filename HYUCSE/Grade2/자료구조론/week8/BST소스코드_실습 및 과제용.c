...
void deleteTreeNode(Node** p, int value)
{
	...
	else
	{
		...
		while (succNode->leftChild != NULL) succNode = succNode->leftChild;
		Node* succParentNode = searchTreeParentNode(*p, succNode->data);

		succParentNode->leftChild = succNode->leftChild;
...
int compareTwoTree(Node* t1, Node* t2)
{
	...
	return compareTwoTree(t1->leftChild, t2->leftChild) && compareTwoTree(t1->rightChild, t2->rightChild);
}